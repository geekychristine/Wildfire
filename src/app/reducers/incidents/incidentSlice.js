...
export const fetchIncidents = createAsyncThunk(
  ...
  async () => {
    const response = fetch(`/incidents/get-incidents-db`)
...
export const selectIncident = (state) =>
  state.incidents.data.selectedIncidentId;
...
export const getSelectedIncident = (state) =>
  state.incidents.data.incidentList.find(
    (incident) => !incident._id.indexOf(selectIncident)
  );
export const getSelectedCase = (state) => {
  const selectedId = selectIncident(state);
  const incidentList = listIncidents(state);
  const selectedIncident = incidentList.find(
  ...
  const selectedCase = caseList.find(
    (cases) => !cases.zip_code.indexOf(caseId)
  );
      ...
      incidentDetails: [
        {
          ...
          rows: [
            ...
            {
              type: "ID",
              content: selectedIncident._id.substr(selectedId.length - 5),
            },
            {
              type: "Initial Time",
              content: `${geographics.time_stamp} ${geographics.time_zone} `,
            },
            {
              type: "Zipcodes Affected",
              content: incident.zip_codes.map((zip) => `${zip}, `),
            },
          ],
        },
        {
          ...
          rows: [
            ...
            {
              type: "Api Keywords",
              content: incident.api_keywords.map((keyword) => `${keyword}, `),
            },
            {
              type: "Property",
              content: incident.property.map((property) => `${property}, `),
            },
      ...
      caseDetails: [
        {
          ...
          rows: [
            ...
            {
              type: "Initial Time",
              content: `${cases.initial_time} ${geographics.time_zone} `,
            },
            {
              type: "Api Keywords",
              content: incident.api_keywords.map((keyword) => `${keyword} ,`),
