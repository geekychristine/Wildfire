...
export const fetchUpdates = createAsyncThunk(
  ...
  async () => {
    const response = fetch(`/updates/get-updates-db`)
...
export const getSelectedUpdate = (state) =>
  state.update.data.updatesList.find(
    (update) => !update._id.indexOf(state.update.data.selectedUpdateId)
  );

export const getUpdatesDetailBlocks = (state) => {
  const selectedId = getUpdateId(state);
  const updatesList = listUpdates(state);
  const selectedUpdate = updatesList.find(
    (update) => !update._id.indexOf(selectedId)
  );
          ...
          rows: [
            {
              ...
              content: updates.incidentUpdate.map(
                (update) => `${update}` || "N/A"
              ),
            },
          ],
